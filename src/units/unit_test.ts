...
describe('Units', () => {
  ...
  describe('Unitless', () => {
    ...
    it('returns error on bad input', () => {
      assert.isFalse(d.dateStringToDuration('f').ok);
    });

    it('clamps input to the range', () => {
      assert.equal(isOK(d.dateStringToDuration('0')), 1, 'clamped from 0 to 1');
    });

    it('rounds to the precision', () => {
      assert.equal(
        isOK(d.dateStringToDuration('1.123456')),
        1.12,
    ...
    it('understands duration shorthands', () => {
      assert.isFalse(d.dateStringToDuration('1w').ok);
      assert.isFalse(d.dateStringToDuration('2d1w').ok);
      assert.isFalse(d.dateStringToDuration('2d3m').ok);
      assert.isFalse(d.dateStringToDuration('1w2d3m').ok);
      assert.isFalse(d.dateStringToDuration('w').ok);
    });
  ...
  describe('Days', () => {
    ...
    it('dateStringToDurations dates correctly', () => {
      assert.equal(isOK(d.dateStringToDuration('2025-01-25')), 3);
    });
