...
describe("allPredecessors", () => {
  it("returns an empty list on a new chart.", () => {
    assert.isEmpty(allPredecessors(0, new Chart()));
    assert.isEmpty(allPredecessors(1, new Chart()));
  });

  it("finds all the correct predecessors", () => {
    const g: DirectedGraph = {
      Vertices: [{}, {}, {}, {}, {}],
      Edges: [
        new DirectedEdge(0, 1),
    ...
    assert.deepEqual(allPredecessors(1, g), [1]);
    assert.deepEqual(allPredecessors(2, g), [2, 1]);
    assert.deepEqual(allPredecessors(3, g), [3, 2, 1]);
  });
  ...
  it("finds all the correct predecessors", () => {
    ...
    assert.deepEqual(allPredecessors(1, g), [1]);
    assert.deepEqual(allPredecessors(2, g), [2, 1]);
    assert.deepEqual(allPredecessors(3, g), [3, 1]);
  });
});
