...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var simulation = (plan2, numSimulationLoops) => {
    const allCriticalPaths = /* @__PURE__ */ new Map();
    for (let i2 = 0; i2 < numSimulationLoops; i2++) {
      const durations = plan2.chart.Vertices.map((t2) => {
  ...
  var criticalPath = [];
  var recalculateSpan = () => {
    const slackResult = ComputeSlack(plan.chart, void 0, precision2.rounder());
    if (!slackResult.ok) {
    ...
    criticalPath = CriticalPath(slacks, precision2.rounder());
  };
  recalculateSpan();
  ...
  var onPotentialCriticialPathClick = (key, allCriticalPaths) => {
  ...
  `;
    ...
    B(criticalPathsTemplate, critialPaths);
