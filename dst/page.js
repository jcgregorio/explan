...
(() => {
  ...
  var applyAllInverseOpsToPlan = (inverses, plan2) => {
    for (let i2 = 0; i2 < inverses.length; i2++) {
      const res = inverses[i2].apply(plan2);
      if (!res.ok) {
        return res;
      }
      plan2 = res.value.plan;
    }
    return ok(plan2);
  };
  var applyAllOpsToPlan = (ops, plan2) => {
    const inverses = [];
    for (let i2 = 0; i2 < ops.length; i2++) {
      const res = ops[i2].apply(plan2);
      if (!res.ok) {
...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var spans = [];
  var criticalPath = [];
  var recalculateSpansAndCriticalPath = () => {
    let slacks = [];
