...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var ActionFromOp = class _ActionFromOp {
    ...
    do(explanMain2) {
      const ret = this.op.apply(explanMain2.plan);
      if (!ret.ok) {
        return ret;
      }
      return ok(
  ...
  var undo = (explanMain2) => {
    ...
    const ret = executeDirectly(action, explanMain2);
    if (!ret.ok) {
      return ret;
    }
  ...
  var executeDirectly = (action, explanMain2) => {
    const ret = action.do(explanMain2);
    if (!ret.ok) {
      return ret;
    }
