...
`;
  ...
  var template = (searchTaskPanel, res = []) => x`
  <input
    type="text"
    @input="${(e2) => {
  ...
  <ul>
    ${res.map(
    (task) => x`<li>${highlightedTarget(task.indexes, task.target)}</li>`
  )}
  </ul>
`;
  var SearchTaskPanel = class extends HTMLElement {
    ...
    onInput(e2) {
      const res = import_fuzzysort.default.go(
        e2.target.value,
