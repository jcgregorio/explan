...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var NewTaskAction = class {
    ...
    undo = true;
    do(explanMain2) {
      let ret = InsertNewEmptyTaskAfterOp(0).apply(explanMain2.plan);
      if (!ret.ok) {
        return ret;
      }
      ret = SetMetricValueOp("Duration", 10, 1).apply(explanMain2.plan);
      if (!ret.ok) {
        return ret;
      }
